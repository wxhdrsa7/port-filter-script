#!/bin/bash
#
# port-filter-with-datasource.sh - 支持IP数据源库管理的端口过滤脚本
# 在原有过滤功能基础上，增加灵活的IP数据源库管理功能
#
# 新增功能：
# - 动态选择和管理IP数据源库
# - 启用/禁用特定数据源
...
DATASOURCE_STATUS_FILE="$CONFIG_DIR/datasource_status.conf"
...
print_usage() {
    cat <<'USAGE'
用法：port-filter-with-datasource [选项]
...
IP数据源库管理功能：
  支持动态选择和管理IP数据源库，避免使用过多库导致的误拦截问题
