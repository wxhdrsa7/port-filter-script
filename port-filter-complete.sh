#!/bin/bash
#
# port-filter-complete.sh - 完整的端口过滤脚本
# 集成所有功能：IP地域过滤、白名单、数据源管理、系统监控等
#
# 版本: 3.0.0
# 作者: AI Assistant
# 功能: 企业级端口访问控制解决方案
...
# 默认IP数据源库
DEFAULT_IP_SOURCES=(
    "metowolf/IPList|https://raw.githubusercontent.com/metowolf/iplist/master/data/country/CN.txt|IPv4|Metowolf的中国IP列表，更新频繁"
    "17mon/ChinaIPList|https://raw.githubusercontent.com/17mon/china_ip_list/master/china_ip_list.txt|IPv4|17mon的中国IP列表，准确性高"
    "gaoyifan/OperatorIP|https://raw.githubusercontent.com/gaoyifan/china-operator-ip/ip-lists/china.txt|IPv4|运营商IP列表，覆盖全面"
    "misakaio/ChinaIP|https://raw.githubusercontent.com/misakaio/chnroutes2/master/chnroutes.txt|IPv4|MisakaIO的中国IP路由表"
)

# 默认IPv6数据源库
DEFAULT_IP6_SOURCES=(
    "gaoyifan/ChinaIPv6|https://raw.githubusercontent.com/gaoyifan/china-operator-ip/ip-lists/china6.txt|IPv6|中国运营商IPv6地址列表"
)
...
initialize_datasources() {
    ...
    if [ ! -f "$DATASOURCE_FILE" ]; then
        print_info "初始化数据源库配置..."
        
        # 添加默认IPv4数据源
        for source in "${DEFAULT_IP_SOURCES[@]}"; do
            echo "$source|enabled" >> "$DATASOURCE_FILE"
        done
        
        # 添加默认IPv6数据源
        for source in "${DEFAULT_IP6_SOURCES[@]}"; do
            echo "$source|enabled" >> "$DATASOURCE_FILE"
        done
        
        chmod 600 "$DATASOURCE_FILE"
    fi
}
...
show_datasources() {
    ...
    printf "%-4s%-30s%-10s%-8s%-50s\n" "$BOLD" "ID" "数据源名称" "类型" "状态" "描述" "$NC"
    printf "%-4s%-30s%-10s%-8s%-50s\n" "" "----" "----------" "----" "----" "--------------------------------------------"
    ...
    while IFS='|' read -r name url ip_type description status; do
        ...
        printf "%-4s%-30s%-10s${status_color}%-8s${NC}%-50s\n" "$id" "$name" "$ip_type" "$status_icon $status" "$description"
        id=$((id + 1))
    done < "$DATASOURCE_FILE"
    ...
    # 显示统计信息
    local ipv4_enabled=$(grep -c "IPv4|enabled$" "$DATASOURCE_FILE" 2>/dev/null || echo "0")
    local ipv6_enabled=$(grep -c "IPv6|enabled$" "$DATASOURCE_FILE" 2>/dev/null || echo "0")
    local ipv4_total=$(grep -c "IPv4" "$DATASOURCE_FILE" 2>/dev/null || echo "0")
    local ipv6_total=$(grep -c "IPv6" "$DATASOURCE_FILE" 2>/dev/null || echo "0")
    
    echo ""
    print_info "数据源统计："
    printf "  IPv4数据源: ${GREEN}%d${NC}/${BLUE}%d${NC} 个启用\n" "$ipv4_enabled" "$ipv4_total"
    printf "  IPv6数据源: ${GREEN}%d${NC}/${BLUE}%d${NC} 个启用\n" "$ipv6_enabled" "$ipv6_total"
}
...
toggle_datasource() {
    ...
    while IFS='|' read -r name url ip_type description status; do
        if [ "$current_id" -eq "$id" ]; then
            ...
            found=true
            print_success "✓ 已将数据源 '$name' 设置为: $new_status"
        else
    ...
    if [ "$found" = false ]; then
        print_error "未找到ID为 $id 的数据源"
        rm -f "$temp_file"
        return 1
    fi
    
    mv "$temp_file" "$DATASOURCE_FILE"
    chmod 600 "$DATASOURCE_FILE"
    
    return 0
}

add_custom_datasource() {
    local name="$1"
    local url="$2"
    local ip_type="$3"
    local description="$4"
    ...
    # 检查是否已存在
    if grep -q "^${name}|" "$DATASOURCE_FILE"; then
        print_warning "数据源 '$name' 已存在"
        return 1
    fi
    
    echo "${name}|${url}|${ip_type}|${description}|enabled" >> "$DATASOURCE_FILE"
    chmod 600 "$DATASOURCE_FILE"
    
    print_success "✓ 已添加自定义数据源: $name"
    return 0
}
...
remove_datasource() {
    ...
    print_success "✓ 已删除数据源: $removed_name"
    return 0
}
...
test_datasource_availability() {
    ...
    print_info "正在测试数据源: $name"
    
    if curl -fsSL --max-time 10 --head "$url" >/dev/null 2>&1; then
        print_success "✓ 数据源可用: $name"
        return 0
    else
        print_error "✗ 数据源不可用: $name"
        return 1
    fi
}

test_all_datasources() {
    print_info "正在测试所有启用的数据源..."
    ...
    while IFS='|' read -r name url ip_type description status; do
        if [ "$status" = "enabled" ]; then
            total=$((total + 1))
            if test_datasource_availability "$name" "$url" >/dev/null 2>&1; then
    ...
    print_info "数据源测试结果："
    printf "  启用数据源: %d\n" "$total"
    printf "  可用数据源: ${GREEN}%d${NC}\n" "$available"
    printf "  不可用数据源: ${RED}%d${NC}\n" "$((total - available))"
    
    if [ "$available" -eq 0 ]; then
        print_error "警告：没有可用的数据源！"
        return 1
    fi
...
download_china_ip() {
    ...
    # 获取启用的IPv4数据源
    local ipv4_sources=($(get_enabled_datasources "IPv4"))
    
    # 下载IPv4列表
    if [ ${#ipv4_sources[@]} -gt 0 ]; then
        if ! download_ip_list "ipv4_sources" "$IPSET_NAME" "中国IPv4"; then
            result=1
        fi
    else
        print_warning "没有启用的IPv4数据源"
    fi
    
    # 获取启用的IPv6数据源
    local ipv6_sources=($(get_enabled_datasources "IPv6"))
    
    # 下载IPv6列表（如果系统支持）
    if [ ${#ipv6_sources[@]} -gt 0 ] && [ -f /proc/net/if_inet6 ]; then
        print_info "检测到IPv6支持，正在下载IPv6列表..."
        if ! download_ip_list "ipv6_sources" "$IPSET_NAME6" "中国IPv6"; then
            print_warning "IPv6列表下载失败，将继续使用IPv4规则"
        fi
    elif [ ${#ipv6_sources[@]} -eq 0 ]; then
        print_warning "没有启用的IPv6数据源"
    fi
...
show_rules() {
    ...
    print_info "数据源配置："
    printf "  IPv4数据源: ${GREEN}%d${NC} 个启用\n" "$ipv4_enabled"
    printf "  IPv6数据源: ${GREEN}%d${NC} 个启用\n" "$ipv6_enabled"
    echo ""
...
datasource_menu() {
    while true; do
        ...
        printf "${GREEN}1.${NC}  查看数据源库\n"
        printf "${GREEN}2.${NC}  启用/禁用数据源\n"
        printf "${GREEN}3.${NC}  添加自定义数据源\n"
        printf "${GREEN}4.${NC}  删除数据源\n"
        printf "${GREEN}5.${NC}  测试数据源可用性\n"
        printf "${GREEN}6.${NC}  重置为默认数据源\n"
        printf "${GREEN}0.${NC}  返回主菜单\n\n"
        printf "${YELLOW}请选择操作 [0-6]: ${NC}"
        ...
        case "$choice" in
            ...
            2)
                show_datasources
                echo ""
                read -rp "请输入要切换的数据源ID: " id
                read -rp "启用还是禁用？(enable/disable): " action
                case "$action" in
                    "enable"|"disable")
                        toggle_datasource "$id" "$action"
                        ;;
                    *)
                        print_error "无效操作，请输入 enable 或 disable"
                        ;;
                esac
                read -rp "按回车继续..." _
                ;;
            3)
                ...
                read -rp "数据源名称: " name
                read -rp "数据源URL: " url
                read -rp "IP类型 (IPv4/IPv6): " ip_type
                read -rp "描述信息: " description
                add_custom_datasource "$name" "$url" "$ip_type" "$description"
                read -rp "按回车继续..." _
                ;;
            4)
                show_datasources
                echo ""
                read -rp "请输入要删除的数据源ID: " id
                read -rp "确认删除？(y/N): " confirm
                if [[ "$confirm" =~ ^[Yy]$ ]]; then
                    remove_datasource "$id"
                fi
                read -rp "按回车继续..." _
                ;;
            5)
                ...
                ;;
            6)
                read -rp "确认重置为默认数据源？(y/N): " confirm
                if [[ "$confirm" =~ ^[Yy]$ ]]; then
                    rm -f "$DATASOURCE_FILE"
                    initialize_datasources
                    print_success "✓ 已重置为默认数据源"
                fi
                read -rp "按回车继续..." _
                ;;
            0)
...
show_menu() {
    ...
    printf "${GREEN}11.${NC} 数据源库管理\n"
    printf "${GREEN}12.${NC} 备份和恢复\n"
    printf "${GREEN}13.${NC} 快速设置向导\n"
    printf "${GREEN}0.${NC}  退出\n\n"
    printf "${YELLOW}请选择操作 [0-13]: ${NC}"
}
...
quick_setup_wizard() {
    ...
    print_info "==================== 快速设置向导 ===================="
    echo ""
    print_info "这个向导将帮助您快速配置基本的端口访问控制规则。"
    echo ""
    
    # 步骤1：选择基本配置模式
    print_info "步骤1：选择配置模式"
    echo ""
    echo "1. 安全模式 - 只开放常用端口，其他全部屏蔽"
    echo "2. 标准模式 - 开放常用端口，可配置特定端口访问控制"
    echo "3. 自定义模式 - 完全自定义配置"
    echo ""
    read -rp "请选择配置模式 [1-3]: " mode_choice
    
    case "$mode_choice" in
        1)
            # 安全模式配置
            print_info "正在应用安全模式配置..."
            ...
            # 屏蔽其他所有端口
            print_info "已开放端口: 22(SSH), 80(HTTP), 443(HTTPS)"
            print_warning "其他所有端口已被屏蔽"
            ;;
        2)
            # 标准模式配置
            print_info "正在应用标准模式配置..."
            ...
            # 配置特定端口
            echo ""
            read -rp "是否需要配置特定端口的访问控制？(y/N): " custom_config
    ...
    # 步骤2：配置自动更新
    echo ""
    read -rp "是否设置自动更新中国IP列表？(y/N): " auto_update
    if [[ "$auto_update" =~ ^[Yy]$ ]]; then
        echo ""
        read -rp "请输入每天更新时间 (HH:MM，默认03:30): " update_time
        update_time=${update_time:-"03:30"}
        set_setting "AUTO_UPDATE_TIME" "$update_time"
        set_setting "AUTO_UPDATE_ENABLED" "true"
        print_success "✓ 已设置每天 $update_time 自动更新"
    fi
