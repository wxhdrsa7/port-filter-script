#!/bin/bash
#
# port-filter-improved.sh - 改进版端口访问控制脚本
# 基于原脚本优化，增加安全性、稳定性和功能性
...
clear_port_rules() {
    ...
    local sets=("$IPSET_NAME" "$IPSET_NAME6")
...
apply_rule() {
    ...
    if [ "$protocol" = "udp" ] || [ "$protocol" = "both" ]; then
        if [ "$mode" = "blacklist" ]; then
            ...
            print_success "✓ UDP端口 $port: 已设置黑名单（阻止中国IPv4）"
        elif [ "$mode" = "whitelist" ]; then
            iptables -I INPUT -p udp --dport "$port" -m set --match-set "$IPSET_NAME" src -j ACCEPT
            iptables -I INPUT -p udp --dport "$port" -j DROP
            print_success "✓ UDP端口 $port: 已设置白名单（仅允许中国IPv4）"
        fi
    fi
...
show_rules() {
    ...
    if [ -f /proc/net/if_inet6 ]; then
        ...
        ip6tables -L INPUT -n -v --line-numbers | grep "tcp dpt:" | head -20
        echo ""
        
        print_warning "IPv6 UDP 规则："
        ip6tables -L INPUT -n -v --line-numbers | grep "udp dpt:" | head -20
        echo ""
    fi
...
clear_all_rules() {
    ...
    # 清除IPv4规则
    iptables -S INPUT | grep -E "$IPSET_NAME|$IPSET_NAME6" | cut -d" " -f2- | while read -r rule; do
    ...
    # 清除IPv6规则
    if [ -f /proc/net/if_inet6 ]; then
        ip6tables -S INPUT | grep -E "$IPSET_NAME|$IPSET_NAME6" | cut -d" " -f2- | while read -r rule; do
